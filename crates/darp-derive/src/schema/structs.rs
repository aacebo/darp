...
pub fn derive(input: &syn::DeriveInput, data: &syn::DataStruct) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::darp::schema::ToSchema for #ident #type_generics #where_generics {
            fn to_schema(&self) -> ::darp::Schema {
                let mut schema = ::darp::schema::object();
                #(#field_stmts)*
                schema.into()
            }
        }
        ...
        impl #impl_generics ::darp::Validate for #ident #type_generics #where_generics {
            fn validate(&self) -> Result<::darp::Value, ::darp::schema::ValidError> {
