...
macro_rules! valueof {
    ...
    ($value:tt as number) => { $crate::value::Number::from($value) };
    ($value:tt as int)    => { $crate::value::Int::from($value) };
    ($value:tt as uint)   => { $crate::value::UInt::from($value) };
    ($value:tt as float)  => { $crate::value::Float::from($value) };
    ($value:tt as bool)   => { $crate::value::Bool::from($value) };
    ($value:tt as string) => { $crate::value::Str::from($value) };
    ($value:tt as object) => { $crate::value::Object::from($value) };
...
mod tests {
    ...
    #[test]
    fn as_number() {
        let n: crate::value::Number = valueof!(42_i32 as number);
        assert!(n.is_int());
    ...
    #[test]
    fn as_int() {
        let i: crate::value::Int = valueof!(42_i32 as int);
        assert_eq!(i.to_i32(), 42);
    }

    #[test]
    fn as_uint() {
        let u: crate::value::UInt = valueof!(42_u32 as uint);
        assert_eq!(u.to_u32(), 42);
    }

    #[test]
    fn as_float() {
        let f: crate::value::Float = valueof!(3.14_f64 as float);
        assert_eq!(f.to_f64(), 3.14);
    }

    #[test]
    fn as_bool() {
        let b: crate::value::Bool = valueof!(true as bool);
        assert_eq!(b.to_bool(), true);
    }

    #[test]
    fn as_string() {
        let s: crate::value::Str = valueof!("hello" as string);
    ...
    fn as_number_from_variable() {
        let x = 42_i32;
        let n: crate::value::Number = valueof!(x as number);
        assert_eq!(n.to_i32(), 42);
    }
}
