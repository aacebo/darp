...
impl Value {
    ...
    pub fn get(&self, path: &path::Path) -> Option<Self> {
        let mut value = self.clone();

        for ident in path.iter() {
            value = match ident {
                path::Ident::Key(_) if value.is_struct() => value
                    .as_object()
                    .as_struct()
                    .field(ident.clone())?
                    .to_value(),
                path::Ident::Index(v) if value.is_array() => {
...
mod tests {
    ...
    mod get {
        ...
        fn struct_field() {
            ...
            let path = path::Path::parse("a").unwrap();
            let result = v.get(&path).unwrap();
            assert_eq!(result.to_i32(), 1);

            let path = path::Path::parse("b").unwrap();
            let result = v.get(&path).unwrap();
        ...
        fn array_index() {
            ...
            let path = path::Path::parse("0").unwrap();
            assert_eq!(v.get(&path).unwrap().to_i32(), 10);

            let path = path::Path::parse("1").unwrap();
            assert_eq!(v.get(&path).unwrap().to_bool(), true);

            let path = path::Path::parse("2").unwrap();
        ...
        fn nested_struct_to_array() {
            ...
            let path = path::Path::parse("items/0").unwrap();
            assert_eq!(v.get(&path).unwrap().to_i32(), 42);

            let path = path::Path::parse("items/1").unwrap();
        ...
        fn nested_array_to_struct() {
            ...
            let path = path::Path::parse("0/name").unwrap();
            assert_eq!(v.get(&path).unwrap().as_str(), "alice");

            let path = path::Path::parse("1/name").unwrap();
        ...
        fn missing_key() {
            ...
            let path = path::Path::parse("z").unwrap();
        ...
        fn missing_index() {
            ...
            let path = path::Path::parse("5").unwrap();
            assert!(v.get(&path).is_none());
        }
        ...
        fn nested_key_through_non_object_returns_none() {
            let v = valueof!({ "a": 1_i32 });
            let path = path::Path::parse("a/b").unwrap();
            assert!(v.get(&path).is_none());
        }
    }
