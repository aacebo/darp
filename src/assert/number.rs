...
impl NumberSchema {
    pub fn equals(mut self, value: reflect::Number) -> Self {
        self.0 = self.0.add(Equals::from(value.to_value()).into());
        self
    }

    pub fn options(mut self, options: &[reflect::Number]) -> Self {
        self.0 = self
            .0
    ...
    pub fn min(mut self, min: reflect::Number) -> Self {
        self.0 = self.0.add(Min::from(min).into());
        self
    }

    pub fn max(mut self, max: reflect::Number) -> Self {
...
mod tests {
    ...
    #[test]
    fn validate_equals() {
        let schema = number().equals(reflect::Number::from_i32(42));
        assert!(schema.validate(&42i32.to_value().into()).is_ok());
    ...
    fn validate_options() {
        let schema = number().options(&[
            reflect::Number::from_i32(1),
            reflect::Number::from_i32(2),
            reflect::Number::from_i32(3),
        ]);
    ...
    fn validate_required_and_equals() {
        let schema = number().required().equals(reflect::Number::from_f64(3.14));
        assert!(schema.validate(&3.14f64.to_value().into()).is_ok());
    ...
    #[test]
    fn validate_min() {
        let schema = number().min(reflect::Number::from_i32(5));
    ...
    #[test]
    fn validate_max() {
        let schema = number().max(reflect::Number::from_i32(10));
        assert!(schema.validate(&5i32.to_value().into()).is_ok());
        assert!(schema.validate(&10i32.to_value().into()).is_ok());
    ...
    fn validate_min_and_max() {
        let schema = number()
            .min(reflect::Number::from_i32(1))
            .max(reflect::Number::from_i32(10));
    ...
    fn validate_min_float() {
        let schema = number().min(reflect::Number::from_f64(1.5));
    ...
    fn validate_max_float() {
        let schema = number().max(reflect::Number::from_f64(9.9));
