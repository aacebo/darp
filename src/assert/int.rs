...
impl IntSchema {
    pub fn equals(mut self, value: reflect::Int) -> Self {
        self.0 = self.0.add(Equals::from(value.to_value()).into());
        self
    }

    pub fn options(mut self, options: &[reflect::Int]) -> Self {
        self.0 = self
            .0
    ...
    pub fn min(mut self, min: isize) -> Self {
        self.0 = self
            .0
            .add(Min::from(reflect::Number::from_isize(min)).into());
        self
    }
    ...
    pub fn max(mut self, max: isize) -> Self {
        self.0 = self
            .0
            .add(Max::from(reflect::Number::from_isize(max)).into());
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn validate_equals() {
        let schema = int().equals(reflect::Int::from_i32(42));
        assert!(schema.validate(&42i32.to_value().into()).is_ok());
    ...
    fn validate_options() {
        let schema = int().options(&[
            reflect::Int::from_i32(1),
            reflect::Int::from_i32(2),
            reflect::Int::from_i32(3),
        ]);
    ...
    fn validate_required_and_equals() {
        let schema = int().required().equals(reflect::Int::from_i32(10));
