...
use super::{Template, ast};

pub trait Pipe {
    fn invoke(&self, this: &reflect::Value, args: &[reflect::Value])
    -> ast::Result<reflect::Value>;
}

pub trait Func {
    fn invoke(&self, args: &[reflect::Value]) -> ast::Result<reflect::Value>;
}

#[derive(Default, Clone)]
pub struct Scope {
    vars: BTreeMap<String, Arc<reflect::Value>>,
...
impl Scope {
    ...
    pub fn var(&self, name: &str) -> Option<&reflect::Value> {
    ...
    pub fn set_var(&mut self, name: &str, value: impl reflect::ToValue) -> &mut Self {
        self.vars
...
mod tests {
    ...
    struct ConstFunc(reflect::Value);
    impl Func for ConstFunc {
        fn invoke(&self, _args: &[reflect::Value]) -> ast::Result<reflect::Value> {
            Ok(self.0.clone())
        }
    }
    ...
    fn func_set_and_get() {
        let mut scope = Scope::new();
        scope.set_func("greet", ConstFunc(crate::valueof!("hi")));
