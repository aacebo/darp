...
pub struct ValidError {
    ...
    pub path: crate::path::Path, // "test[1].name"
    pub message: Option<String>, // "length must be at least 1"
    pub errors: Vec<ValidError>,
}

impl ValidError {
    pub fn new(path: crate::path::Path) -> ValidErrorBuilder {
...
impl ValidErrorBuilder {
    pub fn new(path: crate::path::Path) -> Self {
        Self {
...
mod tests {
    ...
    mod display {
        ...
        fn with_rule_and_message() {
            let err = ValidError::new(crate::path::Path::parse("a/b").unwrap())
                .name("required")
        ...
        #[test]
        fn without_rule() {
            let err = ValidError::new(crate::path::Path::parse("a").unwrap())
        ...
        #[test]
        fn without_message() {
            let err = ValidError::new(crate::path::Path::parse("x").unwrap())
                .name("equals")
                .build();
            let output = err.to_string();
        ...
        #[test]
        fn nested_errors() {
            let child1 = ValidError::new(crate::path::Path::parse("a").unwrap())
                .name("required")
                .message("required")
                .build();
            let child2 = ValidError::new(crate::path::Path::parse("a").unwrap())
                .name("equals")
                .message("not equal")
                .build();
            let mut parent = ValidError::new(crate::path::Path::parse("a").unwrap()).build();
    ...
    mod serde {
        ...
        #[test]
        fn roundtrip() {
            let err = ValidError::new(crate::path::Path::parse("x/0").unwrap())
                .name("equals")
