    ...
    fn to_schema(&self) -> Schema {
        match self {
            ...
            crate::Value::Object(o) => match o {
                ...
                crate::value::Object::Array(_) => array().to_schema(),
...
mod tests {
    ...
    mod validate {
        ...
        fn object_dispatches() {
            ...
            map.insert(crate::path::Ident::key("a"), crate::valueof!(1_i32));
        ...
        fn object_field_valid() {
            ...
            let mut map = HashMap::new();
            map.insert(crate::path::Ident::key("name"), crate::valueof!("alice"));
            assert!(schema.validate(&map.to_value().into()).is_ok());
        }
        ...
        fn object_field_invalid_type() {
            ...
            let schema = Schema::Object(object().field("name", string()));
            let mut map = HashMap::new();
            map.insert(crate::path::Ident::key("name"), crate::valueof!(42_i32));
        ...
        fn object_unexpected_field() {
            ...
            let mut map = HashMap::new();
            map.insert(crate::path::Ident::key("name"), crate::valueof!("alice"));
            map.insert(crate::path::Ident::key("extra"), crate::valueof!(1_i32));
            assert!(schema.validate(&map.to_value().into()).is_err());
        }
        ...
        fn object_missing_required_field() {
            ...
            let mut map = HashMap::new();
            map.insert(crate::path::Ident::key("age"), crate::valueof!(30_i32));
            assert!(schema.validate(&map.to_value().into()).is_err());
        }
        ...
        fn object_nested_object() {
            ...
            inner.insert(
                crate::path::Ident::key("street"),
                crate::valueof!("123 Main"),
            );
            let mut outer = HashMap::new();
            outer.insert(crate::path::Ident::key("address"), inner.to_value());
        ...
        fn object_empty_fields_valid() {
            use std::collections::HashMap;
            let schema = Schema::Object(object());
            let map: HashMap<crate::path::Ident, crate::Value> = HashMap::new();
            assert!(schema.validate(&map.to_value().into()).is_ok());
        }
    }
    ...
    mod serde {
        ...
        fn serialize_number_with_rules() {
            let schema = Schema::Number(
                number()
                    .required()
                    .equals(crate::value::Number::from_i32(42)),
            );
            let json = serde_json::to_string(&schema).unwrap();
            let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        ...
        fn serialize_int_with_rules() {
            let schema = Schema::Int(int().required().equals(crate::value::Int::from_i32(10)));
            let json = serde_json::to_string(&schema).unwrap();
            let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        ...
        fn serialize_float_with_rules() {
            let schema = Schema::Float(
                float()
                    .required()
                    .equals(crate::value::Float::from_f64(3.14)),
            );
            let json = serde_json::to_string(&schema).unwrap();
            let v: serde_json::Value = serde_json::from_str(&json).unwrap();
