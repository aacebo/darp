...
/// let n: value::Number = valueof!(42_i32 as number);
/// let i: value::Int = valueof!(42_i32 as int);
/// let b: value::Bool = valueof!(true as bool);
/// let s: value::Str = valueof!("hello" as string);
/// ```
#[macro_export]
macro_rules! valueof {
    (null) => {
    ...
    ({}) => {
        $crate::Value::from_struct(
            std::collections::HashMap::<$crate::path::Ident, $crate::Value>::new(),
        )
    };

    ({ $( $key:literal : $value:tt ),+ $(,)? }) => {{
        let mut map = std::collections::HashMap::<$crate::path::Ident, $crate::Value>::new();
        $(
            map.insert($crate::path::Ident::key($key), $crate::valueof!($value));
        )+
    ...
    ($value:tt as number) => { $crate::reflect::Number::from($value) };
    ($value:tt as int)    => { $crate::reflect::Int::from($value) };
    ($value:tt as uint)   => { $crate::reflect::UInt::from($value) };
    ($value:tt as float)  => { $crate::reflect::Float::from($value) };
    ($value:tt as bool)   => { $crate::reflect::Bool::from($value) };
    ($value:tt as string) => { $crate::reflect::Str::from($value) };
    ($value:tt as object) => { $crate::reflect::Object::from($value) };
...
#[cfg(test)]
mod tests {
    use crate::reflect::ext::StructExt;
    ...
    #[test]
    fn as_number() {
        let n: crate::reflect::Number = valueof!(42_i32 as number);
        assert!(n.is_int());
    ...
    #[test]
    fn as_int() {
        let i: crate::reflect::Int = valueof!(42_i32 as int);
        assert_eq!(i.to_i32(), 42);
    }

    #[test]
    fn as_uint() {
        let u: crate::reflect::UInt = valueof!(42_u32 as uint);
        assert_eq!(u.to_u32(), 42);
    }

    #[test]
    fn as_float() {
        let f: crate::reflect::Float = valueof!(3.14_f64 as float);
        assert_eq!(f.to_f64(), 3.14);
    }

    #[test]
    fn as_bool() {
        let b: crate::reflect::Bool = valueof!(true as bool);
        assert_eq!(b.to_bool(), true);
    }

    #[test]
    fn as_string() {
        let s: crate::reflect::Str = valueof!("hello" as string);
    ...
    fn as_number_from_variable() {
        let x = 42_i32;
        let n: crate::reflect::Number = valueof!(x as number);
        assert_eq!(n.to_i32(), 42);
    }
}
